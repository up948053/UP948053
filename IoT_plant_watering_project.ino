...
#define AIO_KEY         "aio_JjcL35Guk384ZuGEeSoyRahauTbo"   
...
void loop() {
  ...
  Serial.print("Time =");
  Serial.println(Time);
  if (Time == 14){  // if time is 4pm (14h = 4pm) Turn the grow light LED on
    digitalWrite(growLight, HIGH); 
  }
  if (Time == 4){  // if time is 4pm (4h = 4am) Turn the grow light LED OFF
  ...
  if (SoilMoisture < 50){
    ...
    digitalWrite(pump, LOW); // Turn off the Water if soil moisture level is above 70%
  }
